...
import { chipAnchor, dimAnchor, wallAxis, wallOnScreen, type Dim, type LiveDim } from './dims';
...
export function placeInStage(
	...
	const h = Math.max(el.offsetHeight, CHIP_HIT_H_PX);
...
/**
 * A mark's name, so a plan full of squares reads without a legend. It keeps its
 * screen size whatever the zoom, which is why it lives here and not in the SVG
 * with the square it names.
 */
function renderMarkNames(ctx: CtrlContext): void {
	...
	model.landmarks.forEach((m) => {
		...
		const outCm = g.half + (8 + markNameHalfCrossPx(wallAxis(g.wall))) / ctx.scale;
		placeInStage(ctx, chip, {
...
export function renderCtrlLayer(ctx: CtrlContext): HTMLElement | null {
	...
	ctx.dims.forEach((d) => {
		...
		const place = placer(ctx, chipAnchor(d), { parent });
		if (d.readOnly) {
	...
	if (live) addLabel(ctx.dom.doc, live.value + ' cm', 'live-dim', placer(ctx, dimAnchor(live)));
